  ...
  describe('/api/users', function () {
    describe('POST', function () {
      ...
      it('Should reject users with non-string username', function(){
        ...
        return chai.request(app).post('/api/users').send(testUser)
          .then(res => {
            ...
            expect(res).to.be.json;
            expect(res.body).to.be.an('object');
            expect(res.body.message).to.equal('\'username\': Incorrect field type: expected string');
          });
      });

      it('Should reject users with non-string password', function() {
        const testUser = { username, password: 12345675678, fullname };
        ...
        return chai.request(app).post('/api/users').send(testUser)
          .then(res => {
            ...
            expect(res).to.be.json;
            expect(res.body).to.be.an('object');
            expect(res.body.message).to.equal('\'password\': Incorrect field type: expected string');
          });
      });

      it('Should reject users with non-trimmed username', function(){
        const testUser = { username: 'dummyUser ', password, fullname };
      ...
      it('Should reject users with non-trimmed password', function(){
        const testUser = { username, password: ' dummyPass ', fullname };
      ...
      it('Should reject users with password less than 8 characters', function(){
        const testUser = { username, password: 'afsjed', fullname };
      ...
      it('Should reject users with password greater than 72 characters', function(){
        const testUser = { username, password: 'afsjedasdaafsjedasdaafsjedasdaafsjedasdaafsjedasdaafsjedasdaafsjedasdaafsjedasda', fullname };
      ...
      it('Should reject users with duplicate username', function(){
        return User.findOne()
          .then(user => {
            const testUser = { username: user.username, password, fullname };
            return chai.request(app).post('/api/users').send(testUser);
          })
          .then(res => {
